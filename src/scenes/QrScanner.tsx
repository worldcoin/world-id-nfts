...
function Scanner({ onResult }: { onResult: OnResultFunction }) {
	...
	return (
		<div className="absolute inset-0 overflow-clip rounded-none lg:rounded-xl" ref={containerRef}>
			<QrReader {...getQrReaderProps()} onResult={onResult} />

			<svg className="absolute inset-0" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" fill="none">
				<path d={frame} fill="#191C20" fillRule="evenodd" clipRule="evenodd" opacity=".7" />
...
export function QrScanner({ step, setStep, flow, setFlow, setAddress }: SceneProps) {
	const show = useMemo(() => step === Steps.flow && flow === Flows.scan, [flow, step])
	...
	const handleResult = useCallback<OnResultFunction>(
		result => {
			if (step === Steps.flow && flow === Flows.scan && result && validateAddress(result.getText())) {
